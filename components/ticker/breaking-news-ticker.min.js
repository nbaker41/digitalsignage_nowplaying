! function (C) {
	...
	C.breakingNews = function (e, t) {
		...
		var s = C(e),
			...
			b = !1,
			y = function () {
				if (0 < n.length && ("rtl" == a.settings.direction ? l.css({
						right: n.outerWidth()
					}) : l.css({
						left: n.outerWidth()
					})), 0 < r.length) {
					var e = r.outerWidth();
					"rtl" == a.settings.direction ? l.css({
						left: e
					}) : l.css({
						right: e
					})
			...
			k = function () {
				...
				e -= a.settings.scrollSpeed / 2, c.css({
					marginLeft: e
			...
			w = function () {
				"rtl" === a.settings.direction ? c.stop().animate({
			...
			q = function () {
				"rtl" === a.settings.direction ? (0 <= parseInt(c.css("marginRight"), 10) && (c.css({
			...
			x = function () {
				switch (p = !0, a.settings.effect) {
					case "typography":
						c.find("li").hide(), c.find("li").eq(o).width(30).show(), c.find("li").eq(o).animate({
							width: "100%",
							opacity: 1
						}, 1500);
						break;
					case "fade":
						c.find("li").hide(), c.find("li").eq(o).fadeIn();
						break;
					case "slide-down":
					...
					case "slide-right":
						...
						break;
					default:
						c.find("li").hide(), c.find("li").eq(o).show()
				}
			},
			W = function () {
				if (u = !1, a.settings.play) switch (a.settings.effect) {
					case "scroll":
						"rtl" === a.settings.direction ? v() : k();
						break;
					default:
						a.pause(), b = setInterval(function () {
							a.next()
						}, a.settings.delayTimer)
				}
			};
		a.init = function () {
			if (a.settings = C.extend({}, i, t), "fixed-top" === a.settings.position ? s.addClass("bn-fixed-top").css({
					"z-index": a.settings.zIndex
				}) : "fixed-bottom" === a.settings.position && s.addClass("bn-fixed-bottom").css({
					"z-index": a.settings.zIndex
				}), "default" != a.settings.fontSize && s.css({
					"font-size": a.settings.fontSize
				}), "default" != a.settings.themeColor && (s.css({
					"border-color": a.settings.themeColor,
					color: a.settings.themeColor
				}), n.css({
					background: a.settings.themeColor
				})), "default" != a.settings.background && s.css({
				...
				}), s.addClass("bn-effect-" + a.settings.effect + " bn-direction-" + a.settings.direction), y(), "object" == typeof a.settings.source) switch (a.settings.source.type) {
				case "rss":
					"rss2json" === a.settings.source.usingApi ? ((o = new XMLHttpRequest).onreadystatechange = function () {
						if (4 == o.readyState && 200 == o.status) {
							...
							c.empty().append(t), d = c.children("li"), m = c.children("li").length, y(), "scroll" != a.settings.effect && x(), d.find(".bn-seperator").css({
								height: a.settings.height - 2 * a.settings.borderWidth
							}), W()
						}
					}, o.open("GET", "https://api.rss2json.com/v1/api.json?rss_url=" + a.settings.source.url + "&count=" + a.settings.source.limit + "&api_key=" + a.settings.source.rss2jsonApiKey, !0), o.send()) : ((r = new XMLHttpRequest).open("GET", "https://query.yahooapis.com/v1/public/yql?q=" + encodeURIComponent('select * from rss where url="' + a.settings.source.url + '" limit ' + a.settings.source.limit) + "&format=json", !0), r.onreadystatechange = function () {
						if (4 == r.readyState)
							if (200 == r.status) {
							...
							} else c.empty().append('<li><span class="bn-loader-text">' + a.settings.source.errorMsg + "</span></li>")
					}, r.send(null));
					break;
				case "json":
					C.getJSON(a.settings.source.url, function (e) {
						var t = "",
							i = "";
			...
			} else "html" === a.settings.source ? ("scroll" != a.settings.effect && x(), W()) : console.log('Please check your "source" parameter. Incorrect Value');
			var r, o;
			a.settings.play ? g.find("span").removeClass("bn-play").addClass("bn-pause") : g.find("span").removeClass("bn-pause").addClass("bn-play"), s.on("mouseleave", function (e) {
				var t = C(document.elementFromPoint(e.clientX, e.clientY)).parents(".bn-breaking-news")[0];
				C(this)[0] !== t && (!0 === a.settings.stopOnHover ? !0 === a.settings.play && a.play() : !0 === a.settings.play && !0 === u && a.play())
			}), s.on("mouseenter", function () {
				!0 === a.settings.stopOnHover && a.pause()
			}), h.on("click", function () {
				p && (p = !1, a.pause(), a.next())
			}), f.on("click", function () {
				p && (p = !1, a.pause(), a.prev())
			}), g.on("click", function () {
				p && (g.find("span").hasClass("bn-pause") ? (g.find("span").removeClass("bn-pause").addClass("bn-play"), a.stop()) : (a.settings.play = !0, g.find("span").removeClass("bn-play").addClass("bn-pause")))
			}), C(window).on("resize", function () {
				s.width() < 480 ? (n.hide(), "rtl" == a.settings.direction ? l.css({
					right: 0
				}) : l.css({
					left: 0
				})) : (n.show(), "rtl" == a.settings.direction ? l.css({
					right: n.outerWidth()
				}) : l.css({
					left: n.outerWidth()
				}))
